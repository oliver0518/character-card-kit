...
下面是一张**真实生成的示例卡**（示例角色「游历剑客」，5 张 AI 分件 + 本工具组装）：

<p align="center">
  <img src="docs/example_card.jpg" width="640" alt="示例人物卡">
</p>
...
**硬约束**（违反必返工）：
- 六格图必须**横版 3 列×2 行**，方图会导致名牌错位；
- 底色统一浅色 ≈ `(240, 237, 231)`，否则组装后色带不齐；
- 服饰/装备/微表情六格**一律素面，禁画任何文字标签**（文字由本工具后期叠印）；
...
| `acc_names` / `eq_names` / `expr_names` | ✓ | 三组六格的名牌（数量须等于网格格数） |
| `eq_notes` | ✓ | 装备逐条注记（装备六格下方） |
...
| 坑 | 症状 | 解法 |
|----|------|------|
| 六格源图出成方图 | 名牌与格子错位 | 重出**横版 3×2 (1536×1024)**，名牌带间距 = 半行高才对齐 |
